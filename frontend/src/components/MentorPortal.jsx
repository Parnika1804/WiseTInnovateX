...
const MentorPortal = () => {
  ...
  const loadPortal = async (email) => {
    try {
      ...
      try {
        ...
        if (leaderboardRes.data && leaderboardRes.data.length > 0) setRoundsHappened(true);
      } catch (e) {}
      ...
      try {
        ...
        if (configRes.data.status === 'found') {
          ...
          if (totalRounds >= 2 && currentRound === totalRounds) setIsSecondLastRound(true);
        }
      } catch (e) {}
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      {/* Header */}
      <div className="bg-slate-900 dark:bg-slate-950 text-white px-8 py-5 border-b border-slate-800">
