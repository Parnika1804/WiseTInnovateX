...
const MentorManager = () => {
  ...
  const [assignRow, setAssignRow] = useState(null); // for unassigned mentors
  ...
  // Reassign assigned mentor to a different team
  ...
  // Assign unassigned mentor to any team
  ...
  // For reassign: show ALL mentors so committee can pick any mentor for a team
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Warning if unassigned mentors exist */}
      {unassignedCount > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
          <span className="text-2xl">⚠️</span>
          <p className="text-sm font-semibold text-amber-800">
            {unassignedCount} mentor{unassignedCount > 1 ? 's are' : ' is'} unassigned. Use the <strong>Assign</strong> button to manually assign them to a team.
          </p>
        </div>
      )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        {uploadStatus && <div className={`mt-3 p-3 rounded-lg text-sm font-medium ${uploadStatus.startsWith('✅') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>{uploadStatus}</div>}
        {linkStatus && <div className={`mt-3 p-3 rounded-lg text-sm font-medium ${linkStatus.startsWith('✅') ? 'bg-violet-50 text-violet-700 border border-violet-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>{linkStatus}</div>}
        {emailStatus && <div className={`mt-3 p-3 rounded-lg text-sm font-medium ${emailStatus.startsWith('✅') ? 'bg-indigo-50 text-indigo-700 border border-indigo-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>{emailStatus}</div>}
        {reassignStatus && <div className={`mt-3 p-3 rounded-lg text-sm font-medium ${reassignStatus.startsWith('✅') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>{reassignStatus}</div>}
      </div>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {mentor.assigned_team_id ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-600 border border-red-200">⚠️ Unassigned</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        {mentor.assigned_team_id ? (
                          // ASSIGNED — show Reassign (change which mentor covers this team)
                          ...
                          // UNASSIGNED — show Assign button to pick a team
                          assignRow === mentor.id ? (
                            <div className="flex items-center gap-2">
                              <select
                                value={selectedTeam}
