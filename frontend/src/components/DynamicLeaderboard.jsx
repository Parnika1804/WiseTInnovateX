...
const DynamicLeaderboard = ({ scoringCategories = [], teamData = [], currentRound = 1, onRoundEnded }) => {
  ...
  return (
    <div className="w-full bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden mt-6 relative">
      ...
      {successOverlay && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-[100] w-[90%] max-w-3xl animate-fade-in-up">
          <div className="bg-emerald-600 text-white p-5 rounded-2xl shadow-2xl flex flex-col sm:flex-row items-center justify-between border-4 border-emerald-400 gap-4">
            <div className="flex items-center gap-4 text-center sm:text-left">
              <span className="text-5xl drop-shadow-md">✉️</span>
              <div>
                <h4 className="font-black text-xl mb-1">{successOverlay.title}</h4>
                <p className="text-emerald-50 text-sm font-medium">{successOverlay.body}</p>
              </div>
            </div>
            <button 
              onClick={handleGoToComms} 
              className="bg-white text-emerald-700 px-6 py-3 rounded-xl font-bold text-sm hover:bg-emerald-50 shadow-md transition-colors whitespace-nowrap flex-shrink-0"
            >
      ...
      {/* Header Container */}
      <div className="bg-gradient-to-r from-slate-50 to-blue-50/30 border-b border-slate-200/80 p-6">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-full text-xs font-bold uppercase tracking-wider">
                ...
                return (
                  <tr key={team.id || team.team_id} className={rowBg}>
                    ...
                    <td className="px-6 py-4 font-bold text-slate-900">
                      ...
                      {!team.is_qualified && (
                        <span className="ml-3 text-xs font-semibold text-red-500 bg-red-50 px-2 py-0.5 rounded border border-red-100">
                          Eliminated
                        </span>
                      )}
                    </td>
